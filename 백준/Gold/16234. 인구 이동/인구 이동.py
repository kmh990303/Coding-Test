...
def bfs(i, j, visited):
    ...
    avg_population = total_population // len(countries)  # 버림은 몫 구하는 걸로 대체 가능
...
while True:
    moved = False
    visited = [[False] * n for _ in range(n)]  # 각 국가 방문 여부를 체크

    for i in range(n):
        for j in range(n):
            if not visited[i][j]:  # 아직 방문하지 않은 국가에서 bfs 시작
                if bfs(i, j, visited):
                    moved = True
